...
import { Github, Eye, EyeOff, Trash2 } from "lucide-react";
import {
	Dialog,
	DialogBody,
	DialogContent,
...
import { githubConfigStorage } from "@/services/github-sync";
...
export function GitHubSyncDialog({
	...
	useEffect(() => {
		if (!isOpen) return;
		setShowToken(false);
		githubConfigStorage.get().then((config) => {
			if (config) {
	...
	const handleSave = async () => {
		...
		try {
			const existing = await githubConfigStorage.get();
	...
	const handleClear = async () => {
		try {
			await githubConfigStorage.remove();
			setOwner("");
			setRepo("");
			setPath("data/projects.json");
			setToken("");
				...
				<DialogBody>
					...
					<div className="space-y-3 rounded-lg border bg-muted/30 p-4">
						...
						<div className="space-y-1.5">
							...
							<Input
								...
								placeholder={t("e.g. data/projects.json")}
							/>
					...
					<div className="space-y-3 rounded-lg border bg-muted/30 p-4">
						...
						<div className="space-y-1.5">
							...
							<p className="text-xs text-muted-foreground">
								{t("Requires")} <code className="rounded bg-muted px-1 py-0.5 text-xs">repo</code>{" "}
								{t("scope.")}{" "}
								<a
